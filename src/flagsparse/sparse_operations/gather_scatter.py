...
def _cupy_gather_detect_layout(dense_vector):
    ...
    if (
        ...
        and dense_vector.dtype in (torch.float16, torch.bfloat16)
    ):
        # complex32 alignment with half2 / nv_bfloat162 storage.
        return "complex16_pair"
    raise TypeError(
        "Unsupported gather input format. Expected one of: "
        "1D float16/bfloat16, 1D complex64, or 2D (N,2) float16/bfloat16."
    )
...
def flagsparse_gather_cupy(
    ...
    layout, _ = _cupy_gather_validate_inputs(dense_t, indices_t)
    _cupy_gather_validate_combo(dense_t, indices_t, layout)
    gather_meta = {
    ...
    if nnz == 0:
        gathered_t = _cupy_gather_empty(layout, dense_t.dtype, dense_t.device)
        ...
        dense_raw_t = _cupy_gather_dense_to_raw_torch(dense_t, layout)
        ...
        torch.cuda.synchronize()
        start_time = time.perf_counter()
        stream_ptr = torch.cuda.current_stream(device=dense_t.device).cuda_stream
        try:
        ...
        gathered_t = _cupy_gather_raw_to_dense_torch(out_raw_t, layout, dense_t.dtype)
...
def cusparse_spmv_gather_cupy(dense_vector, indices, selector_matrix=None):
    ...
    dense_t = dense_t.contiguous()
    indices_t = indices_t.contiguous()
    layout, dense_size = _cupy_gather_validate_inputs(dense_t, indices_t)

    selector_dtype = _cupy_gather_selector_dtype(layout, dense_t.dtype)
    if selector_matrix is None:
    ...
    try:
        ...
        start_time = time.perf_counter()
        if layout in ("scalar16", "complex64"):
            gathered_t = _cusparse_spmv(selector_matrix, dense_t)
        elif layout == "complex16_pair":
            gathered_real = _cusparse_spmv(selector_matrix, dense_t[:, 0])
            gathered_imag = _cusparse_spmv(selector_matrix, dense_t[:, 1])
