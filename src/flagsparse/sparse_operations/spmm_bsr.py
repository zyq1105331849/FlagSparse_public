...
def _prepare_spmm_bsr_matrix(data, indices, indptr, shape, block_dim):
    ...
    if not data.is_cuda or not indices.is_cuda or not indptr.is_cuda:
        raise ValueError("data, indices, and indptr must be ROCm/CUDA tensors")
    if indices.device != data.device or indptr.device != data.device:
        raise ValueError("indices and indptr must be on the same ROCm/CUDA device as data")
...
def _validate_spmm_bsr_B(B, prepared, op_code):
    ...
    if not B.is_cuda:
        raise ValueError("B must be a ROCm/CUDA tensor")
    if B.device != prepared.data.device:
        raise ValueError("B must be on the same ROCm/CUDA device as sparse matrix data")
...
def _select_block_n(n_dense_cols, dtype, device=None):
    strategy = resolve_spmm_dcu_launch_strategy(
        "dcu",
    ...
    if strategy.block_n is not None:
        return int(strategy.block_n)
...
def flagsparse_spmm_bsr(
    ...
    if not out.is_cuda:
        raise ValueError("out must be a ROCm/CUDA tensor")
    if out.device != result.device:
        raise ValueError("out must be on the same ROCm/CUDA device as the result")
