...
def _spmm_csr_alg2_compute(plan, B, *, accuracy=False, high_precision=False):
    if B.ndim != 2:
        raise ValueError("B must be a 2D dense tensor")
    if not B.is_cuda:
        raise ValueError("B must be a CUDA tensor")
...
def _run_spmm_csr_alg2_accuracy_hp_route(prepared, B, *, timing=False, diagnostics=False):
    return _run_spmm_csr_alg2_accuracy_impl(
        prepared,
        B,
        timing=timing,
...
def _run_spmm_csr_alg2_accuracy_impl(
    ...
    *,
    ...
    if timing:
        ...
        start.record()
    C = _spmm_csr_alg2_compute(plan, B, accuracy=True, high_precision=high_precision)
    if timing:
        end.record()
        torch.cuda.synchronize()
        compute_ms = start.elapsed_time(end)
...
def flagsparse_spmm_csr_run(
    ...
    *,
    ...
    dense_layout = _normalize_dense_layout(dense_layout)
    dense_layout_algorithms = {"csr_base", "csr_base_accuracy"}
    if dense_layout == "col" and algorithm.name not in dense_layout_algorithms:
        raise SpmmCsrAlgorithmUnavailable(
            "col-major layout is currently supported only by csr_base and csr_base_accuracy"
        )
    ...
    if algorithm.name in dense_layout_algorithms:
...
def benchmark_spmm_opt_case(
    ...
    if run_cusparse:
        try:
            ...
            cu_ms = sparse_ref["ms"]
            sparse_ref_reason = sparse_ref.get("reason")
        except Exception as exc:
    ...
    return {
        ...
        "backend_status": {
            ...
            "cusparse_unavailable_reason": sparse_ref_reason,
        },
        "samples": {
            "base": base_values,
            "opt": opt_values,
...
def benchmark_spmm_case(
    ...
    """Benchmark Triton CSR SpMM vs PyTorch sparse.mm and hipSPARSE CSR @ dense."""
    op_code = _normalize_spmm_op(op)
    op_name = _spmm_op_to_name(op_code)
    device = torch.device("cuda")
    data, indices, indptr = _build_random_csr(
    ...
    if run_cusparse:
        try:
            if op_code != SPMM_OP_NON:
                cusparse_reason = "direct hipSPARSE CSR SpMM reference currently supports op=non only"
            else:
                sparse_ref = _benchmark_spmm_csr_sparse_ref(
                    data,
                    indices,
    ...
    return {
        ...
        "backend_status": {
            ...
            "cusparse_backend": sparse_ref_backend,
...
# DCU direct hipSPARSE CSR SpMM reference helpers.
