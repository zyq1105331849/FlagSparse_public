...
def _prepare_alpha_spmm_alg1_common(data, indices, indptr, shape):
    ...
    if data.dtype not in SUPPORTED_ALPHA_SPMM_ALG1_DTYPES:
        raise TypeError(
            "alpha_spmm_alg1 routes only support float32, float64, "
...
def _run_alpha_spmm_alg1_complex_kernel(prepared, B, out=None):
    C_out = out
    if C_out is None:
        C_out = torch.empty(
...
def _run_alpha_spmm_alg1(prepared, B, meta):
    ...
    if prepared.data.is_complex():
        return _run_alpha_spmm_alg1_complex_kernel(prepared, B)
