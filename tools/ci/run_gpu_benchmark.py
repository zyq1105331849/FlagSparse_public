...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one DCU/ROCm GPU benchmark suite and capture logs/artifacts."
    )
    parser.add_argument(
        "--suite",
        default="quick",
    ...
    parser.add_argument(
        "--matrix-dir",
        default="tests/data",
        help="MatrixMarket file or directory used by mtx-backed suites.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--with-hipsparse",
        action="store_true",
        help="Include hipSPARSE baselines for suites that support them.",
    )
    parser.add_argument(
        "--results-dir",
...
def _command_specs(
    ...
    with_hipsparse = bool(args.with_hipsparse)
    no_sparse_ref = [] if with_hipsparse else ["--no-hipsparse"]
    ...
    commands: Dict[str, List[str]] = {
        ...
        "alpha-spmm-alg1": [
            ...
            "--iters",
            str(args.iters),
            *(["--with-hipsparse"] if with_hipsparse else ["--no-hipsparse"]),
        ],
        "spmm-opt-alg2": [
            ...
            "--iters",
            str(args.iters),
            *([] if with_hipsparse else ["--no-hipsparse"]),
        ],
        "spgemm": [
...
def _gpu_metadata() -> Dict[str, object]:
    ...
    # PyTorch ROCm exposes DCU devices through torch.cuda.
...
def main() -> int:
    ...
    metadata["matrix_dir"] = args.matrix_dir
    metadata["warmup"] = args.warmup
    metadata["iters"] = args.iters
    metadata["with_hipsparse"] = bool(args.with_hipsparse)
