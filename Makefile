...
help:
	@printf '%s\n' \
		...
		'  make gpu-env-check  - validate ROCm/DCU visibility on a GPU runner' \
		'  make gpu-benchmark  - run the quick GPU benchmark suite on a ROCm/DCU machine' \
...
gpu-env-check:
	$(PYTHON) tools/ci/check_gpu_environment.py --require-rocm
