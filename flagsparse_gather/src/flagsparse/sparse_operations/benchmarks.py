...
def _resolve_benchmark_value_dtype(value_dtype):
    if not isinstance(value_dtype, str):
        return value_dtype
    mapping = {
    ...
    if token not in mapping:
        raise ValueError(f"unsupported value dtype token: {value_dtype}")
    return mapping[token]


def _resolve_benchmark_index_dtype(index_dtype):
    ...
    token = index_dtype.strip().lower()
    if token not in mapping:
        raise ValueError(f"unsupported index dtype token: {index_dtype}")
    return mapping[token]
