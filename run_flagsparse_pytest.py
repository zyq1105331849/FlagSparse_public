...
def collect_env_info(project_root: Path) -> dict[str, object]:
    ...
    # PyTorch ROCm intentionally exposes DCU devices through torch.cuda.
    rocm: dict[str, object] = {
        "available": bool(torch.cuda.is_available()),
        "version": getattr(torch.version, "hip", None),
        "device_count": 0,
        "devices": [],
        "rocm_smi": _capture_text(
    ...
    if rocm["available"]:
        try:
            rocm["device_count"] = torch.cuda.device_count()
            devices = []
            for index in range(int(rocm["device_count"])):
                ...
                devices.append(
                    {
                        ...
                        "max_threads_per_block": getattr(
                            props, "max_threads_per_block", None
                        ),
...
def run_performance(
    *,
    ...
    if timed_out:
        ...
        status = "FAIL"
    elif "CUDA is not available" in output or "ROCm/DCU is not available" in output:
        status = "SKIP"
...
def _html_environment_rows(env_info: dict[str, object], generated_at: str) -> list[str]:
    ...
    rocm_info = _env_value(env_info, "rocm", "version")
    device = "rocm" if _env_value(env_info, "rocm", "available") else ""
    devices = _env_value(env_info, "rocm", "devices")
