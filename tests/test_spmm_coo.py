...
def _assert_spmm_coo_matches_reference(data, row, col, B, shape, value_dtype, out=None, block_n=None, block_nnz=DEFAULT_BLOCK_NNZ, op="non"):
...
def print_compare_results(results, value_dtype, index_dtype):
    ...
    print("Compare details (PT-COO / HS-COO / native parity)")
    print("Row/PT is the main default-route diagnostic; Atomic/PT is debug-only.")
    print("-" * 166)
    print(
        f"{'Matrix':<28} {'Row/PT':>7} {'Atomic/PT':>9} {'HS/PT':>7} {'Row/Atomic':>11} "
        f"{'Err(Row/PT)':>12} {'Err(Atomic/PT)':>14} {'Err(HS/PT)':>10} {'Err(Row/Atomic)':>15}"
    )
    print("-" * 166)
    for entry in results:
    ...
    print(
        ...
        f"{'Rowrun':>18} {'Atomic':>18} {'PT':>18} {'HS':>18}"
    )
    print("-" * 178)
...
def run_api_validation_checks():
    if not torch.cuda.is_available():
        print("API checks skipped: ROCm/PyTorch GPU is not available.")
        return 0
...
def run_coo_tile_branch_coverage(warmup=WARMUP, iters=ITERS, run_cusparse=True):
    if not torch.cuda.is_available():
        print("COO branch coverage skipped: ROCm/PyTorch GPU is not available.")
        return 0
...
def _print_synthetic_compare_results(compare_rows):
    ...
    print("-" * 160)
    print(
        f"{'N_rows':>7} {'N_cols':>7} {'NNZ':>10} {'DenseN':>8} {'Row/PT':>7} {'Atomic/PT':>9} {'HS/PT':>7} {'Row/Atomic':>11} "
        f"{'Err(Row/PT)':>12} {'Err(Atomic/PT)':>14} {'Err(HS/PT)':>10} {'Err(Row/Atomic)':>15}"
    )
    print("-" * 160)
