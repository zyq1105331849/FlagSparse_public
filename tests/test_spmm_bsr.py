...
def _time_pytorch_bsr(data, indices, indptr, B, shape, block_dim, op, warmup, iters):
    if op in ("trans", "conj"):
        return None, "PyTorch BSR transpose-family SpMM baseline is unsupported; no fallback", None
...
def _hipsparse_bsr_unavailable_reason():
    return "direct hipSPARSE BSR SpMM reference is not wired for DCU yet"
...
def _time_cusparse_bsr(data, indices, indptr, B, shape, block_dim, op, warmup, iters):
    ...
    if indices.dtype != torch.int32 or indptr.dtype != torch.int32:
        return None, "direct hipSPARSE BSR reference requires a verified hip-python path; no implicit fallback", None
    padded_shape = _padded_shape(shape, block_dim)
...
def _print_notes(run_cusparse):
    ...
    print("PyTorch BSR baseline is attempted only for same-format supported cases; PyTorch BSR transpose-family is recorded as N/A with no fallback.")
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("ROCm/PyTorch GPU is required for native BSR SpMM benchmark")
