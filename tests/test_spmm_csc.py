...
def _hipsparse_csc_unavailable_reason():
    return "direct hipSPARSE CSC SpMM reference is not wired for DCU yet"


def _time_cusparse_csc(data, indices, indptr, B, shape, warmup, iters):
    reason = _hipsparse_csc_unavailable_reason()
    if reason:
...
def _print_notes(run_cusparse):
    ...
    if run_cusparse:
        reason = _hipsparse_csc_unavailable_reason()
        if reason:
            print(f"hipSPARSE CSC baseline: unavailable ({reason}); HS(ms)=N/A.")
        else:
            print("hipSPARSE CSC baseline: HS(ms) uses direct hipSPARSE CSC @ dense with construction outside timing.")
    else:
        print("hipSPARSE CSC baseline disabled by --no-hipsparse; HS(ms)=N/A.")
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("ROCm/PyTorch GPU is required for native CSC SpMM benchmark")
