...
def _print_row(row):
    print(
        ...
        f"{_fmt_reason(row.get('status_reason')):<32}"
    )
...
def build_parser():
    ...
    parser.add_argument(
        "--value-dtypes",
        default=DEFAULT_VALUE_DTYPES,
        help="Comma-separated dtype tokens or 'all' (default: all supported scatter dtypes)",
    )
