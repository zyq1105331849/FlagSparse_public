...
def _timed_spmm_opt_alg2(data, indices, indptr, B, shape, warmup, iters):
    ...
    first, meta = fs.flagsparse_spmm_csr_opt_alg2(
        B=B, prepared=prepared, return_meta=True
    )
    _, elapsed = _benchmark(
...
def run_one_case(
    ...
    summary = {
        ...
        "nnz": int(data.numel()),
        "avg_nnz_per_row": (
            float(data.numel()) / float(n_rows) if n_rows > 0 else 0.0
        ),
        ...
        "sparse_ref_reason": sparse_backend_reason,
    }
...
def print_row(row):
    ...
    if row.get("cusparse_ms") is None and row.get("sparse_ref_reason"):
        print(f"  CU: {row['sparse_ref_reason']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Protected CSR SpMM four-way benchmark with opt_alg2."
    )
    ...
    parser.add_argument(
        "--no-cusparse",
        action="store_true",
        help="Disable sparse backend compare timing",
    )
