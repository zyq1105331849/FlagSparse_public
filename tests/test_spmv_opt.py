...
def _timed_pytorch(data, indices, indptr, x, shape, warmup, iters):
    ...
    try:
        ...
        y = torch.sparse.mm(matrix, x.unsqueeze(1)).squeeze(1)
        op = lambda: torch.sparse.mm(matrix, x.unsqueeze(1)).squeeze(1)
    except Exception:
...
def run_batch(paths, dtype, index_dtype, warmup, iters):
    ...
    for p in paths:
        try:
            ...
            print(f"  ERROR on {os.path.basename(p)}: {exc}")
            continue
        results.append(r)
        print_row(r)
    return results
                ...
                all_rows.append(
                    {
                        ...
                        "opt_vs_base": (
                            r["base_ms"] / r["opt_ms"]
                            if r["opt_ms"] and r["opt_ms"] > 0
                            else None
                        ),
                        "opt_vs_pt": (
                            r["pt_ms"] / r["opt_ms"]
                            if r["pt_ms"] and r["opt_ms"] and r["opt_ms"] > 0
                            else None
                        ),
                        "opt_vs_cu": (
                            r["cu_ms"] / r["opt_ms"]
                            if r["cu_ms"] and r["opt_ms"] and r["opt_ms"] > 0
                            else None
                        ),
                        "err_base": r["err_base"],
                        "err_opt": r["err_opt"],
...
def main():
    ...
    dname = str(dtype).replace("torch.", "")
    print("=" * 170)
    print("FLAGSPARSE SpMV Optimisation A/B Test")
